...
async def clean_tasks_tables(create_tables):
    yield
    async with test_engine.begin() as conn:
        await conn.execute(text("""
            DO $$ BEGIN
                TRUNCATE tasks.task CASCADE;
            EXCEPTION WHEN undefined_table THEN NULL;
            END $$
        """))
        await conn.execute(text("""
            DO $$ BEGIN
                TRUNCATE tasks.habit_task_link CASCADE;
            EXCEPTION WHEN undefined_table THEN NULL;
            END $$
        """))
...
async def captured_events():
    """Подписывается на task.completed и task.overdue, накапливает payload'ы."""
    ...
    for event_name in ("task.completed", "task.overdue"):
