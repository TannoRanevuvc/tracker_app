...
- **Материализация recurring — лениво**, как и просрочка в `tasks`: при каждом GET
  `/transactions` или `/summary` проверяются **все активные `recurring_rule` пользователя**
  с `next_due_date <= сегодня` — по пользователю в целом, независимо от `account_id` или
  других фильтров запроса (фильтры влияют только на то, что возвращается в ответе, не на
  область действия материализации; `/summary` не принимает `account_id` вовсе, но обязан
  материализовать так же). Для каждого просроченного правила создаётся транзакция с
